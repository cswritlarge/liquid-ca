...
TEST_F(ModelTest, expectedDensityIsCorrect)
{
   ...
   for(int i = 0; i < 100; i++)
   {
      Model m(100, 256, 128, 5.0, 1337+i, 0.5);
   ...
   for(int i = 0; i < 100; i++)
   {
      Model m(100, 256, 128, 5.0, 1337+i, 0.25);
      sum += m.CurrentDensity();
   }
   ...
   for(int i = 0; i < 100; i++)
   {
      Model m(100, 256, 128, 5.0, 1337+i, 0.75);
      sum += m.CurrentDensity();
   }
...
{
   Model m0(100, 256, 128, 5.0, 12345, 0.0);
   Model m1(100, 256, 128, 5.0, 12345, 1.0);
...
{
   Model m0(100, 255, 127, 5.0, 12345, 1.0);
   m0.SetPositionalState(0.0);
   EXPECT_EQ(m0.CurrentDensity(), 0.0);

   Model m1(100, 255, 127, 5.0, 12334, 0.0);
...
{
   Model m0(100, 255, 127, 5.0, 1234, 0.9);
...
{
   Model m1(100, 128, 64, 5.0, 1337, 0.5);
   Model m2(100, 128, 64, 5.0, 1338, 0.5);
...
{
   Model m1(0.25, 32, 16, 5.0, 123, 0.5);
}

TEST_F(ModelTest, networkChanges)
{
   Model m(50, 128, 64, 5.0, 1337, 0.5);
   for(int i = 0; i < 25; i++)
   {
...
{
   Model m(100, 10, 5, 1.0, 1234, 0.5, 0.0);
...
{
   Model m(100, 10, 5, 1.0, 1234, 0.5, 0.5);
   auto agents = m.GetAgents();
   m.Step(&identity_rule);
   for(int i = 0; i < agents.size(); i++)
   {
...
{
   Model m(100, 10, 5, 1.0, 1234, 0.5, 200);
